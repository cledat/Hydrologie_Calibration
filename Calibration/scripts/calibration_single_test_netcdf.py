...
#   Read settings file
########################################################################
print('Calibrating process using netcdf file as an enter, please make sure your enter data has the structure of a grdc dataset with in additional the gauges locations and the boolean calibrated in order to select the calibration and the validation station')
...
########nb_station = np.shape(streamflow_data)[1] #recupere le nombre de station
...
gauges = ''
for i in range(len(grdc_calibrated.id)) :
	gauges += '{} {} '.format(grdc_calibrated.gauged_lon[i].values, grdc_calibrated.gauged_lat[i].values) 
...
def RunModel(Individual):
	...
	# Changer la ligne Gauges
	# s assurer de l ordre 

		
	template_xml_new = template_xml_new.replace("%gauges", gauges)	
	...
	if runmodel:
		...
		f.close()
        #---------------------------Add a chmood for each files ---Augustin C -------------------------------------------
		st = os.stat(runfile)
	...
	simulated_streamflow = pandas.read_csv(Qsim_tss,sep=r"\s+",index_col=0,skiprows=3+len(stations),header=None,skipinitialspace=True)
	simulated_streamflow[simulated_streamflow==1e31] = np.nan
   
	if len(observed_streamflow) != len(simulated_streamflow[1]): # Penser a ajouter un shape
...
#	print(observed_streamflow.shape)
...
creator.create("FitnessMin", base.Fitness, weights=(maxDeap,)) #creathe the class for minimization probleme 
creator.create("Individual", array.array, typecode='d', fitness=creator.FitnessMin) # create the fit list deriving from the opti probleme 

toolbox = base.Toolbox()

# Attribute generator
toolbox.register("attr_float", random.uniform, 0, 1) # register a function that generate e number between 0 and 1 

# Structure initializers
toolbox.register("Individual", tools.initRepeat, creator.Individual, toolbox.attr_float, len(ParamRanges)) # Create the individual class
toolbox.register("population", tools.initRepeat, list, toolbox.Individual) # create the class population of indicidual class

def checkBounds(min, max): #Checks the number generated is between min and max, if autside of border, the element take the value of the closer border
	def decorator(func):
...
toolbox.register("evaluate", RunModel)   #enregistre la fonction RunModel comme la fonction permettant de tester un individus 
toolbox.register("mate", tools.cxBlend, alpha=0.15) # combine 2 individual to create 1 individual 
toolbox.register("mutate", tools.mutGaussian, mu=0, sigma=0.3, indpb=0.3) # cree des mutations pour generer de la variabilité
toolbox.register("select", tools.selNSGA2) #  allows to select the individual for next generation 
...
if __name__ == "__main__":
    ...
    ## If some run have alresdy been done, this line will look  at the checkpoint file and continue where it stoped. Thats why it is very important when you begin the calibration to delete the folder runs_calibration
	...
	if os.path.exists(os.path.join(checkpoint)):
		...
		# Numbering of runs
		for ii in range(mu):
			population[ii][-1]= float(gen * 1000 + ii+1) #take the last value of the element of the population et lui donne un nombre surement pour l identifier surement 
	...
	while not any(conditions.values()):
		if startlater == False:
			# Vary the population
			offspring = algorithms.varOr(population, toolbox, lambda_, cxpb, mutpb) ## generate a new individual population based on the last one 

			# put in the number of run
			for ii in range(lambda_):
				offspring[ii][-1] = float(gen * 1000 + ii + 1)

		# saving population in checkpoint file 
		...
		fitnesses = toolbox.map(toolbox.evaluate, invalid_ind) #evaluate the fitness of all indivudual in invalid_ind
		for ind, fit in zip(invalid_ind, fitnesses): # attribute the fitness value to the individual
			ind.fitness.values = fit
		...
		# Select the next generation population Moreover, add the new population to the previous one
	...
	if bestrun:
		...
		Qsim_tss = os.path.join(directory_run,dischargetss)
		simulated_streamflow = pandas.read_table(Qsim_tss,sep=r"\s+",index_col=0,skiprows=3+len(stations),header=None,skipinitialspace=True)
		simulated_streamflow[simulated_streamflow==1e31] = np.nan # before was simulated_streamflow[i]

		#Qsim = simulated_streamflow.values ## Modified taking off the [1]
		#Qsim=np.transpose(np.array([simulated_streamflow[1] , simulated_streamflow[2]]))
		Qsim_array = np.array(simulated_streamflow)
		# Save simulated streamflow to disk
		print(">> Saving \"best\" simulated streamflow (streamflow_simulated_best.csv)")
		Qsim = pandas.DataFrame(data=Qsim_array, index=pandas.date_range(ForcingStart, periods=len(Qsim_array), freq=frequen))
